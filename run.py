...
#test
data_= pd.read_csv(r'shuju.csv',sep=',')
...
all_save_data = np.concatenate((res,y_pred_1),axis=1)
all_save_data_pd = pd.DataFrame(all_save_data,columns=['x','y','PGA',
 'river','fault','lithology','Ht','road','NDVI','x大地','y大地','年总降','landslide'])
 # np.savetxt('data.csv',all_save_data,delimiter=',')
all_save_data_pd.to_csv('resTest.csv',index=False)
# y_test_1.shape

from collections import Counter
cnt=Counter(y_pred.flatten())
cnt[0]