...
data = pd.read_csv(r'分级后（滑坡885千米）.csv', sep=',', encoding='gbk')
...
for i in range(n_classes):
    fpr[i], tpr[i], thresholds = roc_curve(y_one_hot[:, i], y_two_hot[:, i])
    roc_auc[i] = metrics.auc(fpr[i], tpr[i])
# 方法1
# fpr["micro"], tpr["micro"], thresholds = roc_curve(y_one_hot.ravel(), y_two_hot.ravel())
# roc_auc["micro"] = metrics.auc(fpr["micro"], tpr["micro"])
# print("micro：",roc_auc["micro"])
# 方法2
# First aggregate all false positive rates
all_fpr = np.unique(np.concatenate([fpr[i] for i in range(n_classes)]))
# Then interpolateolate all ROC curves at this points
...
for i in range(n_classes):
    ...
    roc_auc[i] = metrics.auc(fpr[i], tpr[i])
# 方法1
# print('调用函数auc：',metrics.roc_auc_score(y_one_hot,y_two_hot, average='micro'))
